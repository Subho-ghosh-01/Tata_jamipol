...
use Session;
use Mail;
class VendorHYRController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Session::get('user_sub_typeSession') == 3) {
            $hyr_lists = DB::table('vendor_hyr')->orderBy('id', 'desc')->get();
        } elseif (Session::get('clm_role') == 'hr_dept') {
            $hyr_lists = DB::table('vendor_hyr')->where('status', 'Pending_with_hr')->orderBy('id', 'desc')->get();
        } else {
            $hyr_lists = DB::table('vendor_hyr')->where('vendor_id', Session::get('user_idSession'))->orderBy('id', 'desc')->get();
        }
        return view('admin.vendor_hyr.index', compact('hyr_lists'));
    }
    ...
    public function create()
    {
        ...
        $previousMonthYear = date('F-Y', strtotime('-1 month'));  // e.g. "April-2025"
        ...
        $check_uploaded = DB::table('vendor_hyr')
        ...
        $check_status = DB::table('vendor_hyr')->select('status')
            ->where('vendor_id', Session::get('user_idSession'))
            ->whereMonth('month', $currentMonth)
            ->whereYear('month', $currentYear)
            ->orderBy('id', 'desc')
        ...
        $setting_date = DB::table('settings')->where('type', 'half_yearly1')->first();
        $setting_date2 = DB::table('settings')->where('type', 'half_yearly2')->first();
        $today_day = date('Y-m');

        if (($today_day == $setting_date->value) || ($today_day == $setting_date->value)) {
            $islast_date = "no";
        } else {
            $islast_date = "yes";
        }
        ...
        return view('admin.vendor_hyr.create', compact(
            'divisions',
            'currentMonthYear',
            'vendorName',
            'vendorlist',
    ...
    public function store(Request $request)
    {
        ...
        $inserted = DB::table('vendor_hyr')->insert([
        ...
        if ($inserted) {
            ...
            $user = [
                'name' => 'HR Department',
                'email' => $recipients,
                'subject' => "Vendor Half Yearly Return Uploaded - Pending Approval ($crm)",
                'Month' => $crm,
