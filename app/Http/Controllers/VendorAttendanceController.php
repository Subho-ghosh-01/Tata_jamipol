...
class VendorAttendanceController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Session::get('user_sub_typeSession') == 3) {
            $attendance_lists = DB::table('vendor_attendance_upload')->orderBy('id', 'desc')->get();
        } elseif (Session::get('clm_role') == 'hr_dept') {
            $attendance_lists = DB::table('vendor_attendance_upload')->where('status', 'Pending_with_hr')->orderBy('id', 'desc')->get();
        } elseif (Session::get('clm_role') == 'Account_dept') {
            $attendance_lists = DB::table('vendor_attendance_upload')->where('status', 'completed')->orderBy('id', 'desc')->get();
        } else {
            $attendance_lists = DB::table('vendor_attendance_upload')->where('vendor_id', Session::get('user_idSession'))->orderBy('id', 'desc')->get();
        }
        return view('admin.vendor-attendance.index', compact('attendance_lists'));
    }
    ...
    public function create()
    {
        ...
        $check_uploaded = DB::table('vendor_attendance_upload')
        ...
        $check_status = DB::table('vendor_attendance_upload')->select('status')
            ->where('vendor_id', Session::get('user_idSession'))
            ->whereMonth('month', $currentMonth)
            ->whereYear('month', $currentYear)
            ->orderBy('id', 'desc')
            ->first();
        $setting_date = DB::table('settings')->where('type', 'attendance_last_day')->first();
        $today_day = date('d');
        ...
        if ($check_status) {
            if ($check_status->status == 'Pending_with_hr') {
                ...
                $color = '#cbce28';
            } elseif ($check_status->status == 'Pending_with_account') {
                $form_status = 'Your Application is pending with Account Team';
                $color = '#0397ec';
            } elseif ($check_status->status == 'completed') {
                $form_status = 'Your Application is Approved by HR ';
                $color = '#31a11b';
        ...
        return view('admin.vendor-attendance.create', compact(
            'divisions',
            'currentMonthYear',
            'vendorName',
            'vendorlist',
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'document1.required' => 'Wage Register is required.',
            'document1.mimes' => 'Only PDF, Excel, JPG, JPEG, PNG, DOC, DOCX files are allowed.',
            'document1.max' => 'Wage Register must be less than 5MB.',
            'document2.required' => 'Bank Details is required.',
            'document2.mimes' => 'Only PDF, Excel, JPG, JPEG, PNG, DOC, DOCX files are allowed.',
            'document2.max' => 'Bank Details must be less than 5MB.'
        ]);
        ...
        $isUploaded = $check_uploaded ? 'yes' : 'no';
        if ($isUploaded == 'yes') {
            return back()->with('message', 'Wage Register has already been uploaded.');
        }
        // Upload the file
        ...
        $location = 'documents/vendor_attendance/';
        ...
        $currentDate = date('Y-m-d', strtotime('-1 month')); //wage month
        // Insert into database
        $inserted = DB::table('vendor_attendance_upload')->insert([
            ...
            'div_id' => Session::get('user_DivID_Session') ?? '0'
        ]);
        ...
        if ($inserted) {
            ...
            $user = [
                'name' => 'HR Department',
                'email' => $recipients,
                'subject' => "Vendor Attendance Uploaded - Pending Approval ($crm)",
                'Month' => $crm,
            ...
            Mail::send('admin.vendor-attendance.send_mail', [
                'data' => $user
            ...
            return back()->with('message', 'Wage Register uploaded successfully.');
        } else {
    ...
    public function edit($attendanceIDenc)
    {
        ...
        $vendor_attendance = DB::table('vendor_attendance_upload')->where('id', $id)->first();

        if (Session::get('user_sub_typeSession') == 2) {
            $lastMonth = Carbon::now()->subMonth();

            $check_uploaded_reject = DB::table('vendor_attendance_upload')
                ->where('vendor_id', Session::get('user_idSession'))
...
<a href="' . route('admin.vendor_attendance.create') . '" class="text-primary fw-bold ms-1">Click here to re-upload your details</a>.';
        ...
        return view('admin.vendor-attendance.edit', compact('vendor_attendance', 'check_uploaded_reject', 'inform_message'));
    ...
    public function update(Request $request, $id)
    {
        ...
        $record = DB::table('vendor_attendance_upload')->where('id', $id)->orderBy('id', 'desc')->first();
        ...
        if ($record->status === 'Pending_with_hr') {
            ...
            $updated = DB::table('vendor_attendance_upload')->where('id', $id)->update([
                'hr_by' => $userId,
            ...
            if ($request->action == 'approve') {
                ...
                $user = [
                    ...
                    'subject' => "Vendor Attendance Uploaded - Approved ($crm)",
                    'Month' => $crm,
                    'vendor_name' => $vendor_name->name ?? 'NA',
                    'vendor_code' => $vendor_name->vendor_code ?? 'NA',
                ...
                $user = [
                    'name' => $vendor_name->name ?? 'NA',
                    'email' => $vendor_name->email,
                    'subject' => "Vendor Attendance Uploaded - Rejected ($crm)",
                    'Month' => $crm,
                    'vendor_name' => $vendor_name->name ?? 'NA',
                    'vendor_code' => $vendor_name->vendor_code ?? 'NA',
                    'doc_status' => 'Rejected',
        ...
        } elseif ($record->status === 'Pending_with_account') {
            ...
            $updated = DB::table('vendor_attendance_upload')->where('id', $id)->update([
                'account_by' => $userId,
                'account_remarks' => $request->remarks,
                'account_decision' => $request->action,
                'account_decision_datetime' => $now,
                'status' => $status,
            ]);
            ...
            if ($request->action == 'approve') {
                ...
                $user = [
                    ...
                    'vendor_code' => $vendor_name->vendor_code ?? 'NA',
                    'doc_status' => 'Rejected',
                    'send_to' => "account_to_vendor_cancel",
                    'remarks' => $request->remarks
                ];
            }
