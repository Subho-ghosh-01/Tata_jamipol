...
class VendorECMController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Session::get('user_sub_typeSession') == 3) {
            $ecm_lists = DB::table('vendor_ecm')->orderBy('id', 'desc')->get();
        } elseif (Session::get('clm_role') == 'hr_dept') {
            $ecm_lists = DB::table('vendor_ecm')->where('status', 'Pending_with_hr')->orderBy('id', 'desc')->get();
        } else {
            $ecm_lists = DB::table('vendor_ecm')->where('vendor_id', Session::get('user_idSession'))->orderBy('id', 'desc')->get();
        }
        return view('admin.vendor_ecm.index', compact('ecm_lists'));
    }
    ...
    public function create()
    {
        ...
        $check_uploaded = DB::table('vendor_ecm')
        ...
        $check_status = DB::table('vendor_ecm')->select('status')
            ->where('vendor_id', Session::get('user_idSession'))
            ->whereMonth('month', $currentMonth)
            ->whereYear('month', $currentYear)
            ->orderBy('id', 'desc')
        ...
        $setting_date = DB::table('settings')->where('type', 'bonus_month')->first();
        $today_month = date('Y-m');

        if ($today_month == $setting_date->value) {
            $islast_date = "no";
        } else {
            $islast_date = "yes";
        }
        ...
        return view('admin.vendor_ecm.create', compact(
            'divisions',
            'currentMonthYear',
            'vendorName',
            'vendorlist',
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'document1.required' => 'Form C is required.',
            'document1.mimes' => 'Only PDF, Excel, JPG, JPEG, PNG, DOC, DOCX files are allowed.',
            'document1.max' => 'From C must be less than 5MB.',
            'document2.required' => 'Form D is required.',
            'document2.mimes' => 'Only PDF, Excel, JPG, JPEG, PNG, DOC, DOCX files are allowed.',
            'document2.max' => 'From D must be less than 5MB.'
        ...
        $inserted = DB::table('vendor_ecm')->insert([
        ...
        if ($inserted) {
            ...
            $user = [
                'name' => 'HR Department',
                'email' => $recipients,
                'subject' => "Vendor ECM Uploaded - Pending Approval ($crm)",
                'Month' => $crm,
            ...
            Mail::send('admin.vendor_ecm.send_mail', [
                'data' => $user
            ...
            return back()->with('message', 'ECM uploaded successfully.');
        } else {
    ...
    public function edit($attendanceIDenc)
    {
        ...
        $vendor_attendance = DB::table('vendor_ecm')->where('id', $id)->first();
...
<a href="' . route('admin.vendor_ecm.create') . '" class="text-primary fw-bold ms-1">Click here to re-upload your details</a>.';
        ...
        return view('admin.vendor_ecm.edit', compact('vendor_attendance', 'check_uploaded_reject', 'inform_message'));
    ...
    public function update(Request $request, $id)
    {
        ...
        $record = DB::table('vendor_ecm')->where('id', $id)->orderBy('id', 'desc')->first();
        ...
        if ($record->status === 'Pending_with_hr') {
            ...
            $updated = DB::table('vendor_ecm')->where('id', $id)->update([
                'hr_by' => $userId,
            ...
            if ($request->action == 'approve') {
                ...
                $user = [
                    ...
                    'email' => $recipients,
                    'subject' => "Vendor ECM Uploaded - Approved ($crm)",
                    'Month' => $crm,
                    'vendor_name' => $vendor_name->name ?? 'NA',
                    'vendor_code' => $vendor_name->vendor_code ?? 'NA',
                ...
                $user = [
                    ...
                    'subject' => "Vendor ECM - Rejected ($crm)",
                    'Month' => $crm,
                    'vendor_name' => $vendor_name->name ?? 'NA',
                    'vendor_code' => $vendor_name->vendor_code ?? 'NA',
                    'doc_status' => 'Rejected',
