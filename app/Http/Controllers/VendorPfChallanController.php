...
class VendorPfChallanController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Session::get('user_sub_typeSession') == 3) {
            $pf_lists = DB::table('vendor_pf_ecr')->orderBy('id', 'desc')->get();
        } elseif (Session::get('clm_role') == 'hr_dept') {
            $pf_lists = DB::table('vendor_pf_ecr')->where('status', 'Pending_with_hr')->orderBy('id', 'desc')->get();
        } else {
            $pf_lists = DB::table('vendor_pf_ecr')->where('vendor_id', Session::get('user_idSession'))->orderBy('id', 'desc')->get();
        }
        return view('admin.vendor_pf_challan.index', compact('pf_lists'));
    }
    ...
    public function create()
    {
        ...
        $check_uploaded = DB::table('vendor_pf_ecr')
        ...
        $check_status = DB::table('vendor_pf_ecr')->select('status')
            ->where('vendor_id', Session::get('user_idSession'))
            ->whereMonth('month', $currentMonth)
            ->whereYear('month', $currentYear)
            ->orderBy('id', 'desc')
            ->first();
        $setting_date = DB::table('settings')->where('type', 'pf_last_day')->first();
        $today_day = date('d');
        ...
        return view('admin.vendor_pf_challan.create', compact(
            'divisions',
            'currentMonthYear',
            'vendorName',
            'vendorlist',
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'document1.required' => 'PF Challan is required.',
            'document1.mimes' => 'Only PDF, Excel, JPG, JPEG, PNG, DOC, DOCX files are allowed.',
            'document1.max' => 'PF Challan must be less than 5MB.',
            'document2.required' => 'ECR is required.',
            'document2.mimes' => 'Only PDF, Excel, JPG, JPEG, PNG, DOC, DOCX files are allowed.',
            'document2.max' => 'ECR must be less than 5MB.'
        ...
        $inserted = DB::table('vendor_pf_ecr')->insert([
        ...
        if ($inserted) {
            ...
            $user = [
                'name' => 'HR Department',
                'email' => $recipients,
                'subject' => "Vendor PF Challan & ECR Uploaded - Pending Approval ($crm)",
                'Month' => $crm,
            ...
            Mail::send('admin.vendor_pf_challan.send_mail', [
                'data' => $user
            ...
            return back()->with('message', 'PF uploaded successfully.');
        } else {
    ...
    public function edit($attendanceIDenc)
    {
        ...
        $vendor_attendance = DB::table('vendor_pf_ecr')->where('id', $id)->first();

        if (Session::get('user_sub_typeSession') == 2) {
            $lastMonth = Carbon::now()->subMonth();
            $check_uploaded_reject = DB::table('vendor_pf_ecr')
                ->where('vendor_id', Session::get('user_idSession'))
...
<a href="' . route('admin.vendor_pf_details.create') . '" class="text-primary fw-bold ms-1">Click here to re-upload your details</a>.';
        ...
        return view('admin.vendor_pf_challan.edit', compact('vendor_attendance', 'check_uploaded_reject', 'inform_message'));
    ...
    public function update(Request $request, $id)
    {
        ...
        $record = DB::table('vendor_pf_ecr')->where('id', $id)->orderBy('id', 'desc')->first();
        ...
        if ($record->status === 'Pending_with_hr') {
            ...
            $updated = DB::table('vendor_pf_ecr')->where('id', $id)->update([
                'hr_by' => $userId,
            ...
            if ($request->action == 'approve') {
                ...
                $user = [
                    ...
                    'email' => $recipients,
                    'subject' => "Vendor PF Challan & ECR Uploaded - Approved ($crm)",
                    'Month' => $crm,
                    'vendor_name' => $vendor_name->name ?? 'NA',
                    'vendor_code' => $vendor_name->vendor_code ?? 'NA',
                ...
                $user = [
                    ...
                    'subject' => "Vendor PF Challan & ECR  - Rejected ($crm)",
                    'Month' => $crm,
                    'vendor_name' => $vendor_name->name ?? 'NA',
                    'vendor_code' => $vendor_name->vendor_code ?? 'NA',
                    'doc_status' => 'Rejected',
