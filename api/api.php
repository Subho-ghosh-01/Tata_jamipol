        ...
        case 'gate-pass-details':
            ...
            if($user_type == 1 && $user_sub_type == 3){
                $fetch_query  = sqlsrv_query($sqlcon, "SELECT * FROM
                    Clms_gatepass WHERE name LIKE '%$entername%'");
                while ($getlist = sqlsrv_fetch_array($fetch_query)) {
                    $AsTemp['employee']    = $getlist['name'];
                    $AsTemp['gatepass']    = $getlist['full_sl'];
                    $AsTemp['designation'] = $getlist['full_sl'];
                    $AsTemp['age']         = $getlist['date_of_birth'];
                    $AsTemp['expiry']      = $getlist['valid_till'];
                    $data[]  = $AsTemp;
                }
            }
            elseif ($user_type == 1 && $user_sub_type == 1) {
                ...
                $fetch_query  = sqlsrv_query($sqlcon, "SELECT * FROM
                    Clms_gatepass WHERE name LIKE '%$entername%'
                    AND created_by IN(".implode(',',$vendorArray).")");
                    while ($getlist = sqlsrv_fetch_array($fetch_query)) {
                        $AsTemp['employee']    = $getlist['name'];
                        $AsTemp['gatepass']    = $getlist['full_sl'];
                        $AsTemp['designation'] = $getlist['job_role'];
                        $AsTemp['age']         = $getlist['date_of_birth'];
                        $AsTemp['expiry']      = $getlist['valid_till'];
                        $data[]  = $AsTemp;
                    }
