...
if ($tokenMatchQuery['token']) {
    ...
    switch ($api) {
        case 'login':
            ...
            $userlogin = sqlsrv_fetch_array(sqlsrv_query($sqlcon, "SELECT id,name,
                user_type,division_id,department_id,user_sub_type FROM userlogins WHERE vendor_code='$username' AND password='$password'"));
            $found     = $userlogin['id'];
            ...
            $query_count = "SELECT permits.id as permitid,
                ...
                FROM permits 
                 ...
                 WHERE permits.issuer_id = '$user_id' AND permits.status = 'Issued' AND permits.return_status ='Pending' 
                    OR permits.return_status='PPg' AND permits.ppg_userid='$user_id'
                    OR permits.return_status='Pending_area' AND permits.area_clearence_id ='$user_id'
                    ORDER BY permits.id DESC";
        ...
        case 'get-report':
            ...
            // $listOfExpired ="SELECT permits.id as permitid,permits.serial_no,permits.division_id,
            //             permits.created_at,permits.status,permits.return_status,userlogins.id as userid,
            //             userlogins.vendor_code,userlogins.name,userlogins.user_type,
            //             divisions.abbreviation
            //     FROM permits 
    ...
    // elseif($api == "downloadPermit")
    // {
        require_once 'vendor/autoload.php';
        $id =  $_REQUEST['permitid'];
        $query      ="SELECT * FROM permits WHERE id='$id'";
        $runQuery   = sqlsrv_query($sqlcon, $query);
        $permit_p   = sqlsrv_fetch_array($runQuery);

        //$permit_p = Permit::where('id',$id)->get();
        ...
        // $section_p = Section::where('id',@$permit_p[0]->section_id)->first();
        //section data
        $query2 ="SELECT * FROM sections WHERE id='".@$permit_p['section_id']."'";
        $runQuery2   = sqlsrv_query($sqlcon, $query2);
        $section_p   = sqlsrv_fetch_array($runQuery2);

        //division data  generate serial number
        $query3 ="SELECT * FROM divisions WHERE id='".@$permit_p['division_id']."'";
        $runQuery3   = sqlsrv_query($sqlcon, $query3);
        $abb   = sqlsrv_fetch_array($runQuery3);
        $created_date = @$permit_p['created_at'];
        $serial_no = @$permit_p['serial_no'];
        $month = date('m', strtotime($created_date));

        //Job Details
        $query4 ="SELECT * FROM jobs WHERE id='".@$permit_p['job_id']."'";
        $runQuery4   = sqlsrv_query($sqlcon, $query4);
        $job_p   = sqlsrv_fetch_array($runQuery4);

        
        
        //User Details 	Permit Requested by
        $query5 ="SELECT * FROM userlogins WHERE id='".@$permit_p['entered_by']."'";
        $runQuery5   = sqlsrv_query($sqlcon, $query5);
        $user_p   = sqlsrv_fetch_array($runQuery5);

        $query6 ="SELECT * FROM departments WHERE id='".@$user_p['department_id']."'";
        $runQuery6   = sqlsrv_query($sqlcon, $query6);
        $department_nameReq   = sqlsrv_fetch_array($runQuery6);

        $query7 ="SELECT * FROM sections WHERE id='".@$user_p['section_id']."'";
        $runQuery7   = sqlsrv_query($sqlcon, $query7);
        $section_nameReq   = sqlsrv_fetch_array($runQuery7);
        

        //User Details  Issued By
        $query8 ="SELECT * FROM userlogins WHERE id='".@$permit_p['issuer_id']."'";
        $runQuery8   = sqlsrv_query($sqlcon, $query8);
        $issuer_id   = sqlsrv_fetch_array($runQuery8);

        $query9 ="SELECT * FROM departments WHERE id='".@$issuer_id['department_id']."'";
        $runQuery9   = sqlsrv_query($sqlcon, $query9);
        $department_nameIss   = sqlsrv_fetch_array($runQuery9);

        $query10 ="SELECT * FROM sections WHERE id='".@$issuer_id['section_id']."'";
        $runQuery10   = sqlsrv_query($sqlcon, $query10);
        $section_nameIss   = sqlsrv_fetch_array($runQuery10);


    //     $toReturn ='<html>';
    //     $toReturn .='<head>';
    //     $toReturn .='<meta http-equiv="Content-Language" content="en-us">';
    //     $toReturn .='<meta http-equiv="Content-Type" content="text/html; charset=WINDOWS-1252">';
    //     $toReturn .='<title>FROM#EHSMSM/JUSCO/446/4001</title>';
    //     $toReturn .='<style>';
    //         $toReturn .= 'p {
    ...
    //     $toReturn .='<table border="0" width="100%" cellspacing="0" cellpadding="0">';
    // 	$toReturn .='<tr>';
    //         $toReturn .='<td width="35%"><font face="Verdana" size="2">FROM#EHSMSM/JUSCO/446/4001</font></td>';
    // 		$toReturn .='<td width="32%" colspan="2">';
    // 		$toReturn .='<p align="center"><font face="Verdana" size="2">REV. #03</font></td>';
    // 		$toReturn .='<td width="32%">';
    // 		$toReturn .='<p align="right"><font face="Verdana" size="2">Effective Date : 05.07.2018</font></td>';
    //     $toReturn .='</tr>';

    //     $toReturn .='<tr>';
    //         $toReturn .='<td width="35%"><font face="Verdana" size="2"></font></td>';
    //         $toReturn .='<td width="32%" colspan="2">';
    //         $toReturn .='<p align="center"><b><font face="Verdana" size="2">PERMIT TO WORK</font></b></td>';
    // 	$toReturn .='</tr>';
        
    // 	$toReturn .='<tr>';
    //         $toReturn .='<td width="35%"><font face="Verdana" size="2"></font></td>';
    //         $toReturn .='<td width="32%" colspan="2">';
    //         $toReturn .='<p align="center"><font face="Verdana" size="2">Reference Safety Standard 1/2</font></td>';
    //         $toReturn .='<td width="32%">';
    // 		$toReturn .='<p align="right"><font face="Verdana" size="2">  Status : <strong>'. @$permit_p['status'].'</font></td>';
    //     $toReturn .='</tr><br><br>';
        
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4">';
    // 		$toReturn .='<p align="center"><font face="Verdana" size="2">';
    // 		$toReturn .='Section :<strong> '. @$section_p['name'] .',</strong> 
    //             Date:<strong> '. date('d-m-Y h:i:s A', strtotime(@$permit_p['start_date'])) .',</strong> 
    //             Sl. No. : <strong> '. $abb['abbreviation']. '/' .$month. '/' .$serial_no .'</strong></td>';
    //     $toReturn .='</tr>';
    //     $toReturn .='<tr>';
    // 		$toReturn .='<td colspan="2"><font face="Verdana" size="2">Name of the Job/Description of Job with Daily Job Plan : <strong>'.@$job_p['job_title']. '/'. @$permit_p['job_description'].'</strong> </font></td>';
    // 		$toReturn .='<td colspan="2">&nbsp;</td>';
    // 	$toReturn .='</tr>';
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="2"><font face="Verdana" size="2">Work Order No. : <strong>'. @$permit_p['order_no'] .'</strong> </font></td>';
    // 		$toReturn .='<td colspan="2">&nbsp;</td>';
    // 	$toReturn .='</tr>';
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="2"><font face="Verdana" size="2">Name of Working Agency : <strong>'. @$user_p['name'].'</strong></font></td>';
    // 		$toReturn .='<td colspan="2">&nbsp;</td>';
    // 	$toReturn .='</tr>';
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="2"><font face="Verdana" size="2">Work Permitted for Area / Equipment (Name / No): <strong>'.@$permit_p['job_location'].'</strong> </font></td>';
    // 		$toReturn .='<td colspan="2">&nbsp;</td>';
    // 	$toReturn .='</tr>';
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4"><font face="Verdana" size="2">Permit Validity</font></td>';
    // 	$toReturn .='</tr>';
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4">';
    // 		$toReturn .='<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#000000">';
    // 			$toReturn .='<tr>';
    // 				$toReturn .='<td width="375"><font face="Verdana" size="2">Original Permit Valid From</font></td>';
    // 				$toReturn .='<td width="150"><font face="Verdana" size="2">' .$st_time. '</font></td>';
    // 				$toReturn .='<td width="200"><font face="Verdana" size="2">' .$start_date. '</font></td>';
    // 				$toReturn .='<td width="150"><font face="Verdana" size="2">'.$ed_time.' </font></td>';
    // 				$toReturn .='<td width="200"><font face="Verdana" size="2">'.$end_date.'</font></td>';
    // 			$toReturn .='</tr>';
    // 			$toReturn .='<tr>';
    // 				$toReturn .='<td width="368"><font face="Verdana" size="2">1st Renewal Valid From</font></td>';
    // 				$toReturn .='<td width="172"><font face="Verdana" size="2"></font></td>';
    // 				$toReturn .='<td width="237"><font face="Verdana" size="2"></font></td>';
    // 				$toReturn .='<td width="232"><font face="Verdana" size="2"></font></td>';
    // 				$toReturn .='<td><font face="Verdana" size="2"></font></td>';
    // 			$toReturn .='</tr>';
    // 			$toReturn .='<tr>';
    // 				$toReturn .='<td width="368"><font face="Verdana" size="2">2nd Renewal Valid From</font></td>';
    // 				$toReturn .='<td width="172">&nbsp;</td>';
    // 				$toReturn .='<td width="237">&nbsp;</td>';
    // 				$toReturn .='<td width="232">&nbsp;</td>';
    // 				$toReturn .='<td>&nbsp;</td>';
    // 			$toReturn .='</tr>';
    // 		$toReturn .='</table>';
    // 		$toReturn .='</td>';
    // 	$toReturn .='</tr>';
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4"><font face="Verdana" size="2"><input type="checkbox" checked="checked">I UNDERSTAND THE HAZARD INVOLVED &amp; HAVE TAKEN THE 
    // 		NECESSARY PRECAUTIONS/CONTROLS FOR THE JOB</font></td>';
    // 	$toReturn .='</tr>';
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4">';
    // 			$toReturn .='<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#000000">';
    ...
    // 					$toReturn .='<td align="center">&nbsp;</td>';
    // 					$toReturn .='<td colspan="2" align="center"><b><font face="Verdana" size="2">Permit Original</font></b></td>';
    // 					$toReturn .='<td align="center" width="24%" colspan="2"><b>';
    // 					$toReturn .='<font face="Verdana" size="2">Permit Renewal First</font></b></td>';
    // 					$toReturn .='<td align="center" width="21%" colspan="2"><b>';
    // 					$toReturn .='<font face="Verdana" size="2">Permit Renewal Second</font></b></td>';
    // 				$toReturn .='</tr>';
    // 				$toReturn .='<tr>';
    // 					$toReturn .='<td align="center" width="9%">&nbsp;</td>';
    // 					$toReturn .='<td align="center" width="8%"><b><font face="Verdana" size="2">
    // 					Permit Requested by (Exe Agency)</font></b></td>';
    // 					$toReturn .='<td align="center" width="17%"><b><font face="Verdana" size="2">Issued By <br>
    // 					(Owner Agency)</font></b></td>';
    // 					$toReturn .='<td align="center" width="10%"><b><font face="Verdana" size="2">Taken By <br>
    // 					(Working Agency)</font></b></td>';
    // 					$toReturn .='<td align="center" width="16%"><b><font face="Verdana" size="2">Issued By <br>
    // 					(Owner Agency)</font></b></td>';
    // 					$toReturn .='<td align="center" width="9%"><b><font face="Verdana" size="2">Taken By <br>
    // 					(Working Agency)</font></b></td>';
    // 					$toReturn .='<td align="center" width="10%"><b><font face="Verdana" size="2">Issued By <br>
    // 					(Owner Agency)</font></b></td>';
    // 					$toReturn .='<td align="center" width="9%"><b><font face="Verdana" size="2">Taken By <br>
    // 					(Working Agency)</font></b></td>';
    // 				$toReturn .='</tr>';
    // 				$toReturn .='<tr>';
    // 					$toReturn .='<td align="center" width="9%"><b><font face="Verdana" size="2">
    // 					Name<br>
    // 					P.NO.</font></b></td>';
    // 					$toReturn .='<td align="center" width="8%"><font face="Verdana" size="2">'.@$user_p['name']. "<br>" .@$user_p['vendor_code'].'</td>';
    // 					$toReturn .='<td align="center" width="17%"><font face="Verdana" size="2">'.@$issuer_id['name'] ."<br>". $issuer_id['vendor_code'].'</font></td>';
    // 					$toReturn .='<td align="center" width="10%"><font face="Verdana" size="2">'. @$user_p['name'] ."<br>" . $user_p['vendor_code'] .'</font></td>';
    // 					$toReturn .='<td align="center" width="16%"><font face="Verdana" size="2">&nbsp;</font></td>';
    // 					$toReturn .='<td align="center" width="9%"><font face="Verdana" size="2">&nbsp;</font></td>';
    // 					$toReturn .='<td align="center" width="10%"><font face="Verdana" size="2">&nbsp;</font></td>';
    // 					$toReturn .='<td align="center" width="9%"><font face="Verdana" size="2">&nbsp;</font></td>';
    // 				$toReturn .='</tr>';
    // 				$toReturn .='<tr>';
    // 					$toReturn .='<td align="center" width="9%"><b><font face="Verdana" size="2">
    // 					Department/Section</font></b></td>';
    // 					$toReturn .='<td align="center" width="8%"><font face="Verdana" size="2">'. @$department_nameReq['department_name']. "/". @$section_nameReq['name'].'</font></td>';
    // 					$toReturn .='<td align="center" width="17%"><font face="Verdana" size="2">'. @$department_nameIss['department_name']. "/" . @$section_nameIss['name'].'</font></td>';
    // 					$toReturn .='<td align="center" width="10%"><font face="Verdana" size="2">'. @$department_nameReq['department_name']  . "/" . @$section_nameReq['name'] .'</font></td>';
    // 					$toReturn .='<td align="center" width="16%">&nbsp;</td>';
    // 					$toReturn .='<td align="center" width="9%">&nbsp;</td>';
    // 					$toReturn .='<td align="center" width="10%">&nbsp;</td>';
    // 					$toReturn .='<td align="center" width="9%">&nbsp;</td>';
    // 				$toReturn .='</tr>';
    // 				$toReturn .='<tr>';
    // 					$toReturn .='<td align="center" width="9%"><b><font face="Verdana" size="2">
    // 					Signature</font></b></td>';
    // 					$toReturn .='<td align="center" width="8%">&nbsp;</td>';
    // 					$toReturn .='<td align="center" width="17%">&nbsp;</td>';
    // 					$toReturn .='<td align="center" width="10%">&nbsp;</td>';
    // 					$toReturn .='<td align="center" width="16%">&nbsp;</td>';
    // 					$toReturn .='<td align="center" width="9%">&nbsp;</td>';
    // 					$toReturn .='<td align="center" width="10%">&nbsp;</td>';
    ...
    // 					$toReturn .='<td align="center" width="9%"><b><font face="Verdana" size="2">
    //                     Date &amp; Time</font></b></td>';
    //                     if(@$permit_p['request_dt']){
    //                        $reqdt = date('d-m-Y H:i:s A', strtotime(@$permit_p['request_dt']));
    //                     }
    // 					$toReturn .='<td align="center" width="8%"><font face="Verdana" size="2">'. $reqdt.'</font></td>';
    //                     if(@$permit_p['issuer_dt']){
    //                         $issuerdt = date('d-m-Y H:i:s A', strtotime(@$permit_p['issuer_dt']));
    //                     }
    //                     $toReturn .='<td align="center" width="17%"><font face="Verdana" size="2">'. $issuerdt.'</font></td>';
    //                     if(@$permit_p['rec_dt']){
    //                         $recdtt = date('d-m-Y H:i:s A', strtotime(@$permit_p['rec_dt']));
    //                     }
    // 					$toReturn .='<td align="center" width="10%"><font face="Verdana" size="2">'. $recdtt.'</font></td>';
    // 					$toReturn .='<td align="center" width="16%">&nbsp;</td>';
    // 					$toReturn .='<td align="center" width="9%">&nbsp;</td>';
    // 					$toReturn .='<td align="center" width="10%">&nbsp;</td>';
    // 					$toReturn .='<td align="center" width="9%">&nbsp;</td>';
    ...
    // 	$toReturn .='</tr>';
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4"> &nbsp;</td>';
    // 	$toReturn .='</tr>';
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4">';
    // 		$toReturn .='<p align="center"><font face="Verdana" size="2"><b>Gate Pass Details:</b><br></font></td>';
    // 	$toReturn .='</tr>';
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4"></td>';
    // 	$toReturn .='</tr>';
    ...
    // 			$toReturn .='<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#000000">';
    // 				$toReturn .='<tr>';
    // 					$toReturn .='<td width="50"><font face="Verdana" size="2">Employee Name</font></td>';
    // 					$toReturn .='<td width="172"><font face="Verdana" size="2">Gate Pass No.</font></td>';
    // 					$toReturn .='<td width="237"><font face="Verdana" size="2">Designation</font></td>';
    // 					$toReturn .='<td width="237"><font face="Verdana" size="2">Age</font></td>';
    //                 $toReturn .='</tr>';

    //                 $query11 ="SELECT * FROM gate_pass_details WHERE permit_id='$id'";
    //                 $runQuery11     = sqlsrv_query($sqlcon, $query11);
    //                 while($gate_pass_details = sqlsrv_fetch_array($runQuery11)){
    //                     $toReturn .='<tr>';
    //                         $toReturn .='<td width="50"><font face="Verdana" size="2">'.$gate_pass_details['employee_name'].'</font></td>';
    //                         $toReturn .='<td width="172"><font face="Verdana" size="2">'.$gate_pass_details['gate_pass_no'].'</font></td>';
    //                         $toReturn .='<td width="237"><font face="Verdana" size="2">'.$gate_pass_details['designation'].'</font></td>';
    //                         $toReturn .='<td width="237"><font face="Verdana" size="2">'.$gate_pass_details['age'].'</font></td>';
    //                     $toReturn .='</tr>';
    // 				}
    // 			$toReturn .='</table>';	
    // 		$toReturn .='</td>';
    ...
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4">';
    // 		$toReturn .='<p align="center"><font face="Verdana" size="2"><b>INSTRUCTIONS:</b></font></td>';
    // 	$toReturn .='</tr>';
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4">
    // 		</td>';
    ...
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4">';
    // 		$toReturn .='<font face="Verdana" size="2">1. Permit not to be signed by less than 
    // 		Shift in charge / supervisor / officers or person authorized by HOD.<br>
    // 		2. For high risk activity two level signature should be maintained.<br>
    // 		3. Clearance for Hydraulic System should be given after putting off 
    // 		hydraulic system, isolating the valve, pushing the emergency switch, 
    // 		depressurizing the system &amp; draining the accumulators. Lf other 
    // 		equipment of same hydraulic system needs system ON then, isolation of 
    // 		hydraulic valve, depressurizing the line &amp; draining the accumulators 
    // 		will be done before issuing the Work Permit..<br>
    // 		4. Communicate to all concern, whose activity may get affected by the 
    // 		ongoing work.<br>
    // 		5. No any person including supervisor will work more than 12 hours 
    // 		continuously. Supervisor of Working Agency will maintain register 
    // 		indicating list of people engaged in the job and also attendance of 
    // 		their workers. One supervisor will supervise only one job at one time. 
    // 		All persons must gate pass / safety card with their validity. Working 
    // 		Agency (Contractors) can use any person from his company to complete the 
    // 		job within specified time.<br>
    // 		6. Once the work permit will be issued, working agency will be 
    // 		responsible for adherence to requirement of the conditions of the 
    // 		permit. Owner of the area will be required to carry out audit to ensure 
    // 		compliance to requirement any time during execution of the job.<br>
    // 		7. Proximity / 6 Direction Hazards should be assessed and filled by 
    // 		Executing Agency before starting the activity and communicated to 
    // 		working agency.</font></td>';
    // 	$toReturn .='</tr>';
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4">
    // 		&nbsp;</td>';
    ...
    // 			$toReturn .='<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#000000">';
    // 				$toReturn .='<tr>';
    // 					$toReturn .='<td width="50"><font face="Verdana" size="2">Direction</font></td>';
    // 					$toReturn .='<td width="172"><font face="Verdana" size="2">Hazard</font></td>';
    // 					$toReturn .='<td width="237"><font face="Verdana" size="2">Precaution</font></td>';
    //                 $toReturn .='</tr>';
    //                 $query12 ="SELECT * FROM permit_hazards WHERE permit_id='$id'";
    //                 $runQuery12     = sqlsrv_query($sqlcon, $query12);
    //                 while($permit_hazards = sqlsrv_fetch_array($runQuery12)){
    //                     $toReturn .='<tr>';
    //                         $toReturn .='<td width="50"><font face="Verdana" size="2">'.$permit_hazards['dir'].'</font></td>';
    //                         $toReturn .='<td width="172"><font face="Verdana" size="2">'.$permit_hazards['hazard'].'</font></td>';
    //                         $toReturn .='<td width="237"><font face="Verdana" size="2">'.$permit_hazards['precaution'].'</font></td>';
    //                     $toReturn .='</tr>';
    // 				}
    // 			$toReturn .='</table>';	
    // 		$toReturn .='</td>';
    ...
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4">';
    // 		$toReturn .='<p align="center"><font face="Verdana" size="2"><b>SAFETY CHECKLIST</b></font></td>';
    // 	$toReturn .='</tr>';
    // 	$toReturn .='<tr>';
    // 		$toReturn .='<td colspan="4">';
    // 			$toReturn .='<table border="0" width="100%" cellspacing="0" cellpadding="0">';
    // 				$toReturn .='<tr>';
    // 					$toReturn .='<td width="1260">&nbsp;</td>';
    // 					$toReturn .='<td width="20">&nbsp;</td>';
    // 					$toReturn .='<td>&nbsp;</td>';
    // 					$toReturn .='<td width="20">&nbsp;</td>';
    // 				$toReturn .='</tr>';
    //                 $toReturn .='<tr>';
    //                     if(@$permit_p['safe_work'] == "yes") { $safe_work = "Yes";}
    //                     if(@$permit_p['safe_work'] == "no") { $safe_work = "No"; }
    //                     if(@$permit_p['safe_work'] == "na") { $safe_work = "NA"; }
    // 					$toReturn .='<td width="1260"><font face="Verdana" size="2">1. Safe Work 
    // 					Procedure has been made &amp; approved in Form #EHSMS / JUSCO / 446 
    // 					/ 4002 :<b>'. $safe_work .'</font></td>';
    //                 $toReturn .='</tr>';
    //                 $toReturn .='<tr>';
    //                     if(@$permit_p['all_person'] == "yes") { $all_person = "Yes";}
    //                     if(@$permit_p['all_person'] == "no") { $all_person = "No"; }
    //                     if(@$permit_p['all_person'] == "na") { $all_person = "NA"; }
    // 					$toReturn .='<td width="1260"><font face="Verdana" size="2">2. All persons 
    ...
    // 					JUSCO.:<b>'. $all_person.'</b></font></td>';
    //                 $toReturn .='</tr>';
    //                 $toReturn .='<tr>';
    //                     if(@$permit_p['worker_working'] == "yes") { $worker_working = "Yes";}
    //                     if(@$permit_p['worker_working'] == "no") { $worker_working = "No"; }
    //                     if(@$permit_p['worker_working'] == "na") { $worker_working = "NA"; }
    // 					$toReturn .='<td width="1260"><font face="Verdana" size="2">3. Worker working 
    // 					on height must have height pass from NTTF or by any competent 
    // 					authority.:<b>'. $worker_working.'</b> </font></td>';
    //                 $toReturn .='</tr>';
    //                 $toReturn .='<tr>';
    //                     if(@$permit_p['all_lifting_tools'] == "yes") { $all_lifting_tools = "Yes";}
    //                     if(@$permit_p['all_lifting_tools'] == "no") { $all_lifting_tools = "No"; }
    //                     if(@$permit_p['all_lifting_tools'] == "na") { $all_lifting_tools = "NA"; }
    // 					$toReturn .='<td width="1260"><font face="Verdana" size="2">4. All lifting 
    // 					tools/Load bearing tools, tackles &amp; safety appliances are in 
    // 					good condition with valid test certificate.:<b>'.$all_lifting_tools.' </b></font></td>';
    // 				$toReturn .='</tr>';
    //                 $toReturn .='<tr>';
    //                     if(@$permit_p['all_safety_requirement'] == "yes") { $all_safety_requirement = "Yes";}
    //                     if(@$permit_p['all_safety_requirement'] == "no") { $all_safety_requirement = "No"; }
    //                     if(@$permit_p['all_safety_requirement'] == "na") { $all_safety_requirement = "NA"; }
    // 					$toReturn .='<td width="1260"><font face="Verdana" size="2">5. All safety 
    ...
    // 					down area / ground area, Use of full body harness etc.:<b>'. $all_safety_requirement.'</b> </font></td>';
    // 				$toReturn .='</tr>';
    //                 $toReturn .='<tr>';
    //                     if(@$permit_p['all_person_are_trained'] == "yes") { $all_person_are_trained = "Yes";}
    //                     if(@$permit_p['all_person_are_trained'] == "no") { $all_person_are_trained = "No"; }
    //                     if(@$permit_p['all_person_are_trained'] == "na") { $all_person_are_trained = "NA"; }
    // 					$toReturn .='<td width="1260"><font face="Verdana" size="2">6. All persons 
    // 					are trained on Safe Working Procedure (SWP):<b>'. $all_person_are_trained.' </b> </font></td>';
    // 				$toReturn .='</tr>';
    //                 $toReturn .='<tr>';
    //                     if(@$permit_p['ensure_the_appplicablle'] == "yes") { $ensure_the_appplicablle = "Yes";}
    //                     if(@$permit_p['ensure_the_appplicablle'] == "no") { $ensure_the_appplicablle = "No"; }
    //                     if(@$permit_p['ensure_the_appplicablle'] == "na") { $ensure_the_appplicablle = "NA"; }
    // 					$toReturn .='<td width="1260"><font face="Verdana" size="2">7. Ensure the 
    // 					applicable activity check list, signed by permit requester, 
    // 					receiver and verified by permit issuer. Checklist shall be 
    // 					attached with permit to work system.:<b>'. $ensure_the_appplicablle.'</b></font></td>';
    // 				$toReturn .='</tr>';
    // 			$toReturn .='</table>';
    // 		$toReturn .='</td>';
    //     $toReturn .='</tr>';
    ...
    //     if(@$permit_p['power_clearance'] == "on") { $powercls = "Yes";} else{ $powercls = "No"; }
    //     $toReturn .='<font face="Verdana" size="2"><p>Shut Down Required?: '. $powercls.'</p></font>';

    //     if(@$permit_p['vlevel'] == ".132KV") { @$VoltageLevel = "132KV"; } elseif(@$permit_p['vlevel'] == ".33KV") { @$VoltageLevel = "33KV"; } elseif(@$permit_p['vlevel'] == ".11KV") { @$VoltageLevel = "11KV"; } elseif (@$permit_p['vlevel'] == ".LT") { @$VoltageLevel = "LT"; }
    //     $toReturn .='<font face="Verdana" size="2"><p>Select Voltage Level:'. $VoltageLevel .'</p></font>';
       
    //     $query13 = "SELECT * FROM power_shutdowns WHERE id='".$permit_p['issuer_power']."'";
    //     $runQuery13     = sqlsrv_query($sqlcon, $query13);
    //     $iss = sqlsrv_fetch_array($runQuery13);

    //     $toReturn .='<font face="Verdana" size="2"><P>Issue power clearance: '. @$iss['name'].' </P></font>';
    //     $toReturn .='<font face="Verdana" size="2"><P>Electrical license : ' .@$permit_p['electrical_license_issuer'].'</P></font>';
    //     $toReturn .='<font face="Verdana" size="2"><P>Validity Date: '. @$permit_p['validity_date_issuer'] .'</P></font>';

    //     $query14 = "SELECT * FROM power_shutdowns WHERE id='".$permit_p['rec_power']."'";
    //     $runQuery14     = sqlsrv_query($sqlcon, $query14);
    //     $rec = sqlsrv_fetch_array($runQuery14);
    //     $toReturn .='<font face="Verdana" size="2"><P>Receive power clearance: '.@$rec['name'] .' </P></font>';
    //     $toReturn .='<font face="Verdana" size="2"><P>Electrical license : '. @$permit_p['electrical_license_rec'] .'</P></font>';
    //     $toReturn .='<font face="Verdana" size="2"><P>Validity Date: '.@$permit_p['validity_date_rec'] .' </P></font>';
    //     $toReturn .='<font face="Verdana" size="2"><P>HT Power Clearance Number: '. @$permit_p['power_clearance_number'] .'</P></font>';
    //     $toReturn .='<font face="Verdana" size="2"><P>LT Power Clearance Number: '. @$permit_p['lt_power_num'] .'</P></font>';
    //     $toReturn .='<font face="Verdana" size="2"><P>Issuer2 Comment :'. @$permit_p['issuer2comment'] .'</P></font>';
    //         $toReturn .='<tr>';
    //             $toReturn .='<td colspan="4">';
    //             $toReturn .='<p align="center"><font face="Verdana" size="2"><b>Power Clearence Details</b></font></td>';
    //         $toReturn .='</tr>';
    //         $toReturn .='<tr>';
    //             $toReturn .='<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#000000">';
    //                 $toReturn .='<tr>';
    //                     $toReturn .='<td width="50"><font face="Verdana" size="2">Positive Isolation Lock Number</font></td>';
    //                     $toReturn .='<td width="172"><font face="Verdana" size="2">Equipment</font></td>';
    //                     $toReturn .='<td width="237"><font face="Verdana" size="2">Location</font></td>';
    //                 $toReturn .='</tr>';
    //                 $query15 ="SELECT * FROM power_clearences WHERE permit_id='$id'";
    //                 $runQuery15    = sqlsrv_query($sqlcon, $query15);
    //                 while($power_clearances = sqlsrv_fetch_array($runQuery15)){
    //                     $toReturn .='<tr>';
    //                         $toReturn .='<td width="50"><font face="Verdana" size="2">'.$power_clearances['positive_isolation_no'].'</font></td>';
    //                         $toReturn .='<td width="172"><font face="Verdana" size="2">'.$power_clearances['equipment'] .'</font></td>';
    //                         $toReturn .='<td width="237"><font face="Verdana" size="2">'.$power_clearances['location'].'</font></td>';
    //                     $toReturn .='</tr>';
    //                 }
    //             $toReturn .='</table>';		
    //         $toReturn .='</tr><br><br>';
    //         if(@$permit_p['confined_space'] == 'on'){
    //             $confined_space ="Yes";
    //         }
    //         else{
    //             $confined_space ="No";
    //         }
    //     $toReturn .='<font face="Verdana" size="2"><P>Confined Space?: '.$confined_space.'</P></font>';
    //         $toReturn .='<tr>';
    //             $toReturn .='<td colspan="4">';
    //             $toReturn .='<p align="center"><font face="Verdana" size="2"><b>Confined Space Details</b></font></td>';
    //         $toReturn .='</tr>';
    //         $toReturn .='<tr>';
    //             $toReturn .='<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#000000">';
    //                 $toReturn .='<tr>';
    //                     $toReturn .='<td width="50"><font face="Verdana" size="2"> Clearance No </font></td>';
    //                     $toReturn .='<td width="172"><font face="Verdana" size="2">Depth	</font></td>';
    //                     $toReturn .='<td width="237"><font face="Verdana" size="2">Location</font></td>';
    //                 $toReturn .='</tr>';
    //                 $query16 ="SELECT * FROM confined_spaces WHERE permit_id='$id'";
    //                 $runQuery16    = sqlsrv_query($sqlcon, $query16);
    //                 while($confined_spaces = sqlsrv_fetch_array($runQuery16)){
    //                     $toReturn .='<tr>';
    //                     $toReturn .='<td width="50"><font face="Verdana" size="2">'.$confined_spaces['clearance_no'].'</font></td>';
    //                     $toReturn .='<td width="172"><font face="Verdana" size="2">'.$confined_spaces['depth'].'</font></td>';
    //                     $toReturn .='<td width="237"><font face="Verdana" size="2">'.$confined_spaces['location'].'</font></td>';
    //                     $toReturn .='</tr>';
    //                 }
    //             $toReturn .='</table>';		
    //         $toReturn .='</tr><br>';
    //     if(@$permit_p['high_risk'] == 'on'){
    //         $high_risk = "Yes";
    //     }else {
    //         $high_risk = "No";
    //     }
    //     $toReturn .='<font face="Verdana" size="2"><P>High Risk?:'.$high_risk .'</P></font>';
    //     if(@$permit_p['area_clearence_required'] == 'on'){
    //         $area_clearence_required = "Yes";
    //     }else {
    //         $area_clearence_required = "No";
    //     }
    //     $toReturn .='<font face="Verdana" size="2"><P>Area Clearance Required?:'.$area_clearence_required .'</P></font>';
    //     $query17 ="SELECT * FROM userlogins WHERE id='".$permit_p['area_clearence_id']."'";
    //     $runQuery17    = sqlsrv_query($sqlcon, $query17);
    //     $area_clearence_name = sqlsrv_fetch_array($runQuery17);
    //     $toReturn .='<font face="Verdana" size="2"><P>Name for Area Clearance Request: '. @$area_clearence_name['name'] .'</P></font>';
    //     $toReturn .='<font face="Verdana" size="2"><P>Special Instruction: '. @$permit_p['s_instruction'].'</P></font>';
    //     $toReturn .='<font face="Verdana" size="2"><P><input type="checkbox"  checked="checked">&nbsp;Training has been received regarding this job and all queries have been classified. We have  understood all the risks involved in the job and its mitigation plan.</P></font><br>';
    //     $toReturn .='<tr>';
    //         $toReturn .='<td colspan="4">';
    //         $toReturn .='<p align="left"><font face="Verdana" size="2"><b>Return Details : </b></font></td>';
    //     $toReturn .='</tr>';
    //     if(@$permit_p['complete'] == 'yes'){
    //         $complete = "Yes";
    //     }else {
    //         $complete = "No";
    //     }
    //     $toReturn .='<font face="Verdana" size="2"><P>Complete : '. $complete .'</P></font>';
    //     $toReturn .='<font face="Verdana" size="2"><P>Requester Comment : '. @$permit_p['requester_remark'].'</P></font>';
    //     $toReturn .='<font face="Verdana" size="2"><P>Issuer Comment : '. @$permit_p['issuer_remark'].'</P></font>';
    //     $toReturn .='<font face="Verdana" size="2"><P>Area Clearance Comment : '. @$permit_p['area_return_remark'].'</P></font><br>';
    //     $toReturn .='<tr>';
    //         $toReturn .='<td colspan="4">';
    //         $toReturn .='<p align="center"><font face="Verdana" size="2"><b>Time Change Details</b></font></td>';
    //     $toReturn .='</tr>';
    //     $toReturn .='<tr>';
    //         $toReturn .='<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#000000">';
    //             $toReturn .='<tr>';
    //                 $toReturn .='<td width="50"><font face="Verdana" size="2"> SL No. </font></td>';
    //                 $toReturn .='<td width="172"><font face="Verdana" size="2">Old Time</font></td>';
    //                 $toReturn .='<td width="237"><font face="Verdana" size="2">New Time</font></td>';
    //                 $toReturn .='<td width="237"><font face="Verdana" size="2">Remarks</font></td>';
    //             $toReturn .='</tr>';
                
    //         // //old time & new time
    //         $query18 ="SELECT * FROM change_requests WHERE permit_id='$id' AND status ='Approved' AND type='Time'";
    //         $runQuery18    = sqlsrv_query($sqlcon, $query18);
    //         $count= 1;
    //         while($time_comment = sqlsrv_fetch_array($runQuery18)){
    //             $toReturn .='<tr>';
    //                 $toReturn .='<td width="50"><font face="Verdana" size="2">' . $count++ .' </font></td>';
    //                 $toReturn .='<td width="172"><font face="Verdana" size="2">'.date('d-m-Y H:i',strtotime($time_comment['old_time'])) .'</font></td>';
    //                 $toReturn .='<td width="237"><font face="Verdana" size="2">'.date('d-m-Y H:i',strtotime($time_comment['new_time'])) .'</font></td>';
    //                 $toReturn .='<td width="237"><font face="Verdana" size="2">'.$time_comment['issuer_comment'].'</font></td>';
    //             $toReturn .='</tr>';
    //         }
    //         $toReturn .='</table>';	
    //     $toReturn .='</tr>';
    ...
    //     $toReturn .='<tr>';
    //         $toReturn .='<td colspan="4">';
    //         $toReturn .='<p align="center"><font face="Verdana" size="2"><b>Issuer Change Details</b></font></td>';
    //     $toReturn .='</tr>';
    //     $toReturn .='<tr>';
    //         $toReturn .='<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#000000">';
    //             $toReturn .='<tr>';
    //                 $toReturn .='<td width="50"><font face="Verdana" size="2"> SL No. </font></td>';
    //                 $toReturn .='<td width="100"><font face="Verdana" size="2">Old Issuer</font></td>';
    //                 $toReturn .='<td width="100"><font face="Verdana" size="2">New Issuer</font></td>';
    //                 $toReturn .='<td width="210"><font face="Verdana" size="2">Old Issuer Remarks</font></td>';
    //                 $toReturn .='<td width="210"><font face="Verdana" size="2">New Issuer Remarks</font></td>';
    //             $toReturn .='</tr>';
    //         // old isssuer  & new issuer
    //         $query19 ="SELECT * FROM change_requests WHERE permit_id='$id' AND status ='Approved' AND type='Issued'";
    //         $runQuery19    = sqlsrv_query($sqlcon, $query19);
    //         $count1 = 1;
    //         while($issuerComments = sqlsrv_fetch_array($runQuery19)){
    //             $query20 = "SELECT * FROM userlogins WHERE id='".$issuerComments['change_by_id']."'";
    //             $runQuery20  = sqlsrv_query($sqlcon, $query20);
    //             $oldissuername = sqlsrv_fetch_array($runQuery20);
    //             $query21 = "SELECT * FROM userlogins WHERE id='".$issuerComments['new_id']."'";
    //             $runQuery21  = sqlsrv_query($sqlcon, $query21);
    //             $newissuername = sqlsrv_fetch_array($runQuery21);
    //             $toReturn .='<tr>';
    //                     $toReturn .='<td width="50"><font face="Verdana" size="2">' .@$count1++ .'</font></td>';
    //                     $toReturn .='<td width="150"><font face="Verdana" size="2">'.@$oldissuername['name'].'</font></td>';
    //                     $toReturn .='<td width="200"><font face="Verdana" size="2">'.@$newissuername['name'].'</font></td>';
    //                     $toReturn .='<td width="200"><font face="Verdana" size="2">'.@$issuerComments['comment'].'</font></td>';
    //                     $toReturn .='<td width="200"><font face="Verdana" size="2">'.@$issuerComments['issuer_comment'].'</font></td>';
    //             $toReturn .='</tr>';
    //         }
    //         $toReturn .='</table>';	
    //     $toReturn .='</tr>';
