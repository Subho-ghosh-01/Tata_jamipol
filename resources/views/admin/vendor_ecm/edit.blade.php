...
@section('breadcrumbs')
    ...
    <li class="breadcrumb-item"><a href="{{ route('admin.vendor_esic_details.index') }}">List of Vendor Bonus Return /
            Filling Documents</a>
    </li>
    ...
    @section('content')
        ...
        @if(($vendor_attendance->status == 'Pending_with_hr' && Session::get('clm_role') == 'hr_dept'))
            <div class="card-body material-card mt-4 shadow rounded-3" {!! $foemview ?? '' !!}>
                ...
                <form action="{{ route('admin.vendor_ecm.update', $vendor_attendance->id) }}" method="POST" autocomplete="off"
                    id="hr_form">
                    @csrf
                    @method('PUT')
