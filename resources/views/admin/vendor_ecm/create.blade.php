...
@section('breadcrumbs')
    ...
    <li class="breadcrumb-item"><a href="{{route('admin.vendor_esic_details.index')}}">List of Vendor Bonus Return / Filling
            Documents</a>
    </li>
...
@else
    ...
    @section('content')
        ...
        @endif
        @if(session('message'))
            <div class="alert {{ session('message') == 'ECM uploaded successfully.' ? 'alert-success' : 'alert-danger' }}">
                {{ session('message') }}
            </div>
        @endif
        ...
        <form action="{{ route('admin.vendor_ecm.store') }}" method="post" enctype="multipart/form-data" id="form"
            autocomplete="off">
            @csrf
                        ...
                        <div class="custom-file-upload border rounded p-4">
                            <div class="row mb-2">
                                @if(!empty($vendorName->name))
                                    <div class="col-md-3">
                                        <p><strong>Vendor Name:</strong> {{ $vendorName->name }}</p>
                                    </div>
                                @endif
                                <div class="col-md-3">
                            ...
                            <div class="mb-3" {!! $style !!}>
                                ...
                                <select name="vendor_id" id="vendor_id" class="form-select select2" required>
                                    ...
                                    @foreach($vendorlist as $vendor)
                                        <option value="{{ $vendor->id }}" @if(isset($vendorName) && $vendor->id == $vendorName->id)
                                        selected @endif>
                                            {{ $vendor->name }}
                                        </option>
                                    @endforeach
