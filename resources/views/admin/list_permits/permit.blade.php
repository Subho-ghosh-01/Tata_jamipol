...
//owner
$user_p_o          	= UserLogin::where('id',@$permit_p->area_clearence_id)->first();
$department_nameReq_o = Department::where('id',@$user_p_o->department_id)->first();
...
$vendor_supervisors   = VendorSupervisor::where('id',@$permit_p->permit_req_name)->get();
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#000000">
			...
			<tr>
				...
				<td align="center" width="16%"><font face="Arial" size="2">{{@$issuer_id->name}}<br>{{@$issuer_id->vendor_code}} <br>@if (!empty($vendor_supervisors) && !empty($vendor_supervisors->supervisor_name)) {{@$vendor_supervisors->supervisor_name}} @else {{ '' }} @endif</font></td>
				<td align="center" width="12%"><font face="Arial" size="2">@if(@$user_p_o->name)  {{@$user_p_o->name}} @else {{@$issuer_id->name}} @endif<br>@if(@$user_p_o->vendor_code){{@$user_p_o->vendor_code}} @else{{@$issuer_id->vendor_code}} @endif</font></td>
			...
			<tr>
				...
				<td align="center" width="12%"><font face="Arial" size="2">@if(@$department_nameReq_o->department_name) {{@$department_nameReq_o->department_name }} @else {{@$department_nameIss->department_name}} @endif </font></td>
	...
	<tr>
		...
		<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#000000">
			...
			<tr>
				...
				<td align="center" width="29%" height="44"><font face="Arial" size="2">@if(@$permit_p->area_clearence_required =='on') {{@$area_clearence_name->name}}<br>{{@$area_clearence_name->vendor_code}} @else {{@$issuer_id->name}}<br>{{@$issuer_id->vendor_code}} @endif</font></td>
			</tr>
			<tr>
				...
				<td align="center" width="29%"><font face="Arial" size="2">@if(@$permit_p->area_clearence_required =='on'){{ @$areaDepartment->department_name }} @else {{ @$department_nameIss->department_name}} @endif </font></td>
			</tr>
			<tr>
				<td align="center" width="9%"><b><font face="Arial" size="2">
				Signature</font></b></td>
	...
	<tr>
		...
		({{@$issuer_id->name}} {{@$issuer_id->vendor_code}}) <br>Sign. of Permit Requested by </font></b></td>
	</tr>
	<tr>
		<td colspan="4" align="left">
		<div align="center">
	...
	<tr>
		...
		<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#000000">
			...
			<tr>
				...
				<td><font face="Arial" size="2"> @if($gate_pass_details[$key]->intime) {{date('h:i A', strtotime($gate_pass_details[$key]->intime))}}  @endif</td>
				<td><font face="Arial" size="2">
             @if($gate_pass_details[$key]->outtime)   {{date('h:i A', strtotime($gate_pass_details[$key]->outtime))}}
            @endif
              </font></td>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
	...
	<tr>
		...
		<b><font face="Arial" size="2">({{@$user_p->name}} {{@$user_p->vendor_code}}) <br>Sign. of Permit Receiver (Working Agency)</font></b></td>
	</tr>
</table>
