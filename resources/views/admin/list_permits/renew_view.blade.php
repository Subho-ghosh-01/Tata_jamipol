...
<form action="{{ route('admin.renew_issuer') }}" method="post" enctype="multipart/form-data"> 
    ...
    <input type="text" name="renew_id" value="{{$decode}}">
    <div id="first">
        <div class="form-group row">
                            ...
                            @foreach($gate_pass_details as $key => $value)
                                <tr>
                                    ...
                                    <td><input type="text" class="form-control"   value="{{$gate_pass_details[$key]->expirydate}}" readonly="readonly"></td>
                                     <td><input type="text" class="form-control"   value="{{$gate_pass_details[$key]->outtime}}" readonly="readonly"></td>
                                </tr>
                            @endforeach
                        @endif    
                    </tbody>
...
<!---------------------------------------------------------------------------------------------------------------- -->
    <a name="anchor"></a>  @if($permit_division_data->PermitStatus != 'Requested') <div id="second" style="display:block"> @else <div id="second" style="display:none"> @endif
                            ...
                            @foreach($power_clearances as $key => $value)
                                <tbody id="append_power_clearence">
                                    <tr class="remove_tr_power_cls" id="remove_tr_power_cls">
                                        <input type="hidden" class="form-control" name="p_id[]" value="{{$power_clearances[$key]->id}}">
                                        <td><input type="text" class="form-control" value="{{$power_clearances[$key]->equipment}}" readonly></td>
                                        <td><input type="text" class="form-control" value="{{$power_clearances[$key]->positive_isolation_no}}"  readonly></td>
                                        <td><input type="text" class="form-control" value="{{$power_clearances[$key]->location}}"  readonly></td>
                                        <td><input type="text" class="form-control" value="{{$power_clearances[$key]->box_no}}"  readonly></td>
                                        <td><input type="text" class="form-control" value="{{$power_clearances[$key]->caution_no}}"  readonly></td>
                                    </tr>
                                </tbody>
                            @endforeach
                        @endif
        ...
        <?php 
        $renewtbl = RenewPermit::where('id',$decode)->first();  ?>
        <div class="form-group row">
            <label for="form-control-label" class="col-sm-2 col-form-label">Area Clearance Required </label>
            <div class="col-sm-10">
                <input type="checkbox" class="form-control-check" name="area_clearance_req"  id="area_cleck" @if($renewtbl->status == "Pending_Renew_Area") disabled  checked @endif>         
            </div>
        </div>
                ...
                <select class="form-control" name="area_clearence_id" id="emp_id">
                    ...
                    <?php $users = UserLogin::where('id',@$permit_division_data->PermitArea_clearenceId)->first(); ?>
                        @foreach($forAreaClearence as $forArea)
                            <option value="{{@$forArea->id}}">{{@$forArea->name}}</option>
                        @endforeach
                    @endif
                </select>
            </div>
        ...
        <div class="form-group row">
            <div class="col-sm-12 text-center">
                <input type="submit" name="button" class="btn btn-primary" value="Approve">
            </div>
        </div>
        @endif
