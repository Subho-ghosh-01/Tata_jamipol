...
@else
    ...
    @section('content')
        ...
        @if(session('message'))
            <div class="alert {{ session('message') == 'Esic uploaded successfully.' ? 'alert-success' : 'alert-danger' }}">
                {{ session('message') }}
            </div>
        @endif
        ...
        <form action="{{ route('admin.vendor_esic_details.store') }}" method="post" enctype="multipart/form-data" id="form"
            autocomplete="off">
            @csrf
