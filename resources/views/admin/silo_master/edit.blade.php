...
@section('breadcrumbs')
    ...
    <li class="breadcrumb-item"><a href="{{ route('admin.silo_master.index') }}">SILO Master List</a></li>
    <li class="breadcrumb-item active" aria-current="page">Edit Field</li>
@endsection

@section('content')
    <form action="{{ route('admin.silo_master.update', $silo->id) }}" method="post" autocomplete="off">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <input type="text" id="label" name="label" class="form-control"
                value="{{ old('label', $silo->label) }}" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <select id="type" name="type" class="form-control" required>
                ...
                @endphp
                @foreach($types as $t)
                    <option value="{{ $t }}" {{ old('type', $silo->type) == $t ? 'selected' : '' }}>
                        {{ ucfirst($t) }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3 form-check" id="multipleWrapper" style="display:none;">
            <input type="checkbox" id="multiple" name="multiple" value="1" class="form-check-input"
                {{ old('multiple', $silo->ismultiple ?? 0) ? 'checked' : '' }}>
        ...
        <div class="mb-3 form-check">
            <input type="checkbox" id="isrequired" name="required" value="1" class="form-check-input"
                {{ old('isrequired', $silo->isrequired) ? 'checked' : '' }}>
        ...
        <div class="mb-3">
            ...
            <input type="number" id="displayorder" name="order" class="form-control"
                value="{{ old('displayorder', $silo->displayorder ?? 0) }}">
        </div>

        <!-- Active -->
        <div class="mb-3 form-check">
            <input type="checkbox" id="isactive" name="active" value="1" class="form-check-input"
                {{ old('isactive', $silo->isactive) ? 'checked' : '' }}>
