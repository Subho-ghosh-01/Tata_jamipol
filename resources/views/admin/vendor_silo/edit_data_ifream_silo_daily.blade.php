{{-- resources/views/admin/vendor_silo/edit_silo_daily.blade.php --}}
...
<body class="py-5">
        ...
        <div class="card shadow-lg">
            ...
            <div class="card-body p-4">
                ...
                <form id="dynamicForm" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="id" value="{{ $silo_details->id }}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select hd " name="division_id" id="division" required>
                                ...
                                @foreach($divs as $division)
                                    <option value="{{ $division->id }}" {{ $silo_details->division_id == $division->id ? 'selected' : '' }}>
                                        {{ $division->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select hd" name="silo_id" required>
                                ...
                                @foreach($active_list as $list)
                                    <option value="{{ $list->id }}" {{ $silo_details->silo_tanke_id == $list->id ? 'selected' : '' }}>
                                        {{ $list->full_sl }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            @if($silo_details->image)
                                <small>Current: <a href="{{ asset('' . $silo_details->image) }}"
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <textarea class="form-control hd" rows="3"
                                name="remarks_local">{{ $silo_details->remarks_local }}</textarea>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Remarks (English)</label>
                            <textarea class="form-control hd" rows="3"
                                name="remarks_english">{{ $silo_details->remarks_english }}</textarea>
                        </div>
                    </div>
    ...
    @if(($user_check_safety->clm_role == 'Safety_dept' && @$flow->department_id == '2') || (6 == $user_id && $vms->status == 'pending_with_vendor_supervisor'))
    ...
    <script>
        const fields = @json($fieldMasters);
        const savedData = @json($formData); // decoded json from DB
    ...
    <script>
        $(document).ready(function () {
            $("#dynamicForm").on("submit", function (e) {
                ...
                $.ajax({
                    url: "{{ route('silo_daily_inspection.store', '') }}",
                    method: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
    ...
    <script> $('#division').on('change', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    let savedPlant = "{{ $silo_details->section_id ?? '' }}"; // 👈 comes from DB
...
<script>
        ...
        document.getElementById('hr_form').addEventListener('submit', function (e) {
            ...
            fetch('{{ route("silo_daily_inspection.update", $silo_details->id) }}', {
