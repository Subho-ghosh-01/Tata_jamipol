...
@section('content')
    @if(Session::get('user_sub_typeSession') == 5)
        {{-- If unauthorized, show message (redirect should be handled in controller/middleware) --}}
                    ...
                    <div class="tab-pane fade show active" id="activeList" role="tabpanel" aria-labelledby="active-tab">
                        <div class="table-responsive">
                            <table class="table table-striped table-sm" id="activeTable">
                                ...
                                <tbody>
                                    @forelse($vms_lists->where('return_status', '') as $key => $list)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            ...
                                            <td>
                                                @if($list->status == 'approve')
                                                    ...
                                                    @endif
                                                @elseif($list->status == 'pending_with_inclusion_user')
                                            ...
                                            <td>
                                                <a href="{{ route('vendor_silo.edit_entry', $list->id) }}"
                                                    class="btn btn-sm btn-primary">Details</a>

                                                @if($list->status == 'draft')
                                                    <a href="{{ route('vendor_silo.edit', $list->id) }}"
                                                        class="btn btn-sm btn-warning">Edit</a>
                                                @endif

                                                @if($list->created_by == Session::get('user_idSession') && $list->return_status == '')
                                ...
                                <tbody>
                                    @forelse($vms_lists->where('return_status', '!=', '') as $key => $list)
                                        <tr>
                                            ...
                                            <td>
                                                @if($list->status == 'approve')
                                                    @if($list->status == 'approve' && $list->return_status == '')
                                                        <span class="blink-arrow text-success">➡</span>
                                                        <span class="badge bg-success" style="color:white">Active</span>
                                                    @else
                                                        <span class="blink-arrow text-danger">➡</span>
    ...
    <script>
        $(function () {
            ...
            $form.on("submit", function (e) {
                ...
                $.ajax({
                    url: "{{ route('vendor_silo.returnsilo') }}",
                    type: "POST",
                    data: $form.serialize(),
