...
@endsection
@section('content')
<form action="{{route('admin.power_shutdown.update',$powerShutdown->id)}}" method="post"  autocomplete="off">
    @csrf
    @method('PUT')
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            ...
            @if($powerShutdown->user_type == 1)
                <option value="1" @if($powerShutdown->user_type == 1 ) {{ 'selected' }} @endif>Employee</option>
            @endif
            @if($powerShutdown->user_type == 2)
            <option value="2" @if($powerShutdown->user_type == 2 ) {{ 'selected' }} @endif>Vendor</option>
            @endif
            </select>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <input type="text" class="form-control" name="vendor_code"   value="{{$powerShutdown->p_number}}" id="" autocomplete="off" >
        </div>
    </div>
    <div class="form-group row">
        <label for="form-control-label" class="col-sm-2 col-form-label">Name</label>
        <div class="col-sm-10">
            <input type="text" class="form-control" name="name" id="" value="{{$powerShutdown->name}}" >
        </div>
    </div>
    <div class="form-group row">
        <label for="form-control-label" class="col-sm-2 col-form-label">Email</label>
        <div class="col-sm-10">
            <input type="email" class="form-control" name="email" id="" value="{{$powerShutdown->email_id}}">
        </div>
    </div>
    <div id="Employee">
        <?php       
            $div_id  = DB::table('divisions')->where('id',$powerShutdown->division_id)->get(); 
            // echo $div_id  
        ?>
    @if($powerShutdown->user_type == 1)
                    ...
                    <select class="form-control" id="division_id" name="division_id">
                        ...
                        @if($divisions->count() > 0)
                            @foreach($divisions as $division)
                                <option value="{{$division->id}}" @if(@$div_id[0]->id == @$division->id) {{ 'selected' }} @endif> {{$division->name}} </option>
                            @endforeach
                        @endif
                    </select>
                </div>  
                    ...
                    <select class="form-control" id="department_id" name="department_id">
                        ...
                        @if($departments->count() > 0 )
                            @foreach($departments as $department)
                                <option value="{{@$department->id}}" @if(@$powerShutdown->department_id == $department->id) {{'selected'}} @endif> {{$department->department_name}} </option>
                            @endforeach
                        @endif 
                    </select>
                </div>
                    ...
                    <select class="form-control" id="section_id" name="section_id">
                        ...
                        @if($sections->count() > 0 )
                            @foreach($sections as $section)
                                <option value="{{$section->id}}" @if(@$powerShutdown->section_id == $section->id) {{'selected'}} @endif> {{$section->name}} </option>
                            @endforeach
                        @endif 
        ...
        @endif
            <div class="form-group row">
                ...
                <div class="col-sm-10" id="">
                    <input type="text" class="form-control" name="electrical_license_emp" id="" value="{{@$powerSTedits[0]->electrical_license}}">&nbsp;
                </div>
            </div>
            <div class="form-group row">
                <label for="form-control-label" class="col-sm-2 col-form-label">License Number Validity date</label>
                <div class="col-sm-10" id="">
                    <input type="date" class="form-control" name="validity_date_emp" id="edate" value="{{@$powerSTedits[0]->validity_date}}">&nbsp;
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <table style="width: 180px;">
                        <tr><td><span>132KV</span></td>
                            <td><input type="radio" name="v132kv_emp" @if($powerSTedits[0]->kv132 == 'yes') {{'checked'}} @endif value="yes">&nbsp; Yes
                            <input type="radio" name="v132kv_emp" @if($powerSTedits[0]->kv132 == 'no') {{'checked'}} @endif  value="no">&nbsp; No
                            </td>
                        <tr>
                        <tr><td><span>33KV</span></td>
                            <td><input type="radio" name="v33kv_emp" @if($powerSTedits[0]->kv33 == 'yes') {{'checked'}} @endif value="yes">&nbsp; Yes
                            <input type="radio" name="v33kv_emp" @if($powerSTedits[0]->kv33 == 'no') {{'checked'}} @endif value="no">&nbsp; No
                            </td>
                        <tr><td><span>11KV</span></td>
                            <td><input type="radio" name="v11kv_emp" @if($powerSTedits[0]->kv11 == 'yes') {{'checked'}} @endif  value="yes">&nbsp; Yes
                            <input type="radio" name="v11kv_emp" @if($powerSTedits[0]->kv11 == 'no') {{'checked'}} @endif value="no">&nbsp; No
                            </td>
                        <tr><td><span>LT</span></td>
                            <td><input type="radio" name="vlt_emp"  @if($powerSTedits[0]->lt == 'yes') {{'checked'}} @endif value="yes">&nbsp; Yes
                            <input type="radio" name="vlt_emp" @if($powerSTedits[0]->lt == 'no') {{'checked'}} @endif value="no">&nbsp; No
                            </td>
                        <tr>
                    </table>
                </div>
            ...
            <div class="form-group row">
                ...
                <div class="form-check col-sm-10">
                        <input type="radio" class="" name="issue_power" @if(@$powerSTedits[0]->issue_power == 'yes') {{'checked'}} @endif value="yes">&nbsp; Yes
                        <input type="radio" class=""  name="issue_power" @if(@$powerSTedits[0]->issue_power == 'no') {{'checked'}} @endif value="no">&nbsp; No
                </div>
            </div>
            <div class="form-group row">
                <label for="form-control-label" class="col-sm-2 col-form-label">Receive Power Clearance</label>
                <div class="form-check col-sm-10">
                    <input type="radio" class="" name="rec_power" @if(@$powerSTedits[0]->receive_power == 'yes') {{'checked'}} @endif value="yes">&nbsp; Yes
                    <input type="radio" class=""  name="rec_power" @if(@$powerSTedits[0]->receive_power == 'no') {{'checked'}} @endif value="no">&nbsp; No  
                </div>
            </div>
        </div>
    @endif
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-12">
            <table class="table table-bordered">
                ...
                @if($powerSTedits->count() > 0)
                    @php $count=1 @endphp
                        @foreach($powerSTedits as $key => $value)
                            <tr>
                                <td>{{$count++}}</td>
                                <td>{{$powerSTedits[$key]->supervisor_name}}</td>
                                <td>{{$powerSTedits[$key]->electrical_license}}</td>
                                <td>{{$powerSTedits[$key]->validity_date}}</td>                 
                                <td>
                                    <table>
                                        <tr>
                                        <td>132 KV</td>
                                        <td>@if($powerSTedits[$key]->kv132 == 'yes') {{"Yes"}} @else {{"No"}} @endif</td>
                                        </tr>
                                        <tr>
                                        <td>33 KV</td>
                                        <td>@if($powerSTedits[$key]->kv33 == 'yes') {{"Yes"}} @else {{"No"}} @endif</td>
                                        </tr>
                                        <tr>
                                        <td>11 KV</td>
                                        <td>@if($powerSTedits[$key]->kv11 == 'yes') {{"Yes"}} @else {{"No"}} @endif</td>
                                        </tr>
                                        <tr>
                                        <td>LT</td>
                                        <td>@if($powerSTedits[$key]->lt == 'yes') {{"Yes"}} @else {{"No"}} @endif</td>
                                        </tr>
                                    </table>
                                </td>
                                <td>@if($powerSTedits[$key]->issue_power == 'yes') {{"Yes"}} @else {{"No"}} @endif</td> 
                                <td>@if($powerSTedits[$key]->receive_power == 'yes') {{"Yes"}} @else {{"No"}} @endif</td> 
                                <td><a class="btn btn-info btn-sm" href="{{ route('admin.childShutdown',\Crypt::encrypt($powerSTedits[$key]->child_id)) }}">Edit</a></td> 
                            </tr>
                        @endforeach
                @else
                    <tr>
    ...
    <!-- this code for insert new record -->
    <div class="form-group row" id="sup">
        <!-- <label for="form-control-label" class="col-sm-2 col-form-label">Enter Details</label> -->
        <div class="col-sm-11">
            <table class="table table-bordered">
                <thead> 
                ...
                <tbody id="dataview">
                    <tr class="appendrow" id="">
                        ...
                        <td><input type="date" class="form-control" name="validity_date_ven[]"></td>
