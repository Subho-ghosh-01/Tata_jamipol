...
@section('breadcrumbs')
    ...
    <li class="breadcrumb-item"><a href="{{route('admin.vendor_esic_details.index')}}">List of Vendor Half Yearly Return
            Documents</a>
    </li>
...
@else
    ...
    @section('content')
        ...
        @endif
        @if(session('message'))
            <div class="alert {{ session('message') == 'Document uploaded successfully.' ? 'alert-success' : 'alert-danger' }}">
                {{ session('message') }}
            </div>
        @endif
        ...
        <form action="{{ route('admin.vendor_hyr.store') }}" method="post" enctype="multipart/form-data" id="form"
            autocomplete="off">
            @csrf
