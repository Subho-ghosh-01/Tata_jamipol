...
@section('breadcrumbs')
    ...
    <li class="breadcrumb-item"><a href="{{ route('admin.vendor_hyr.index') }}">List of Vendor Half Yearly Return</a>
    </li>
    ...
    @section('content')
        ...
        @if(($vendor_attendance->status == 'Pending_with_hr' && Session::get('clm_role') == 'hr_dept'))
            <div class="card-body material-card mt-4 shadow rounded-3" {!! $foemview ?? '' !!}>
                ...
                <form action="{{ route('admin.vendor_hyr.update', $vendor_attendance->id) }}" method="POST" autocomplete="off"
                    id="hr_form">
                    @csrf
                    @method('PUT')
