...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        ...
        <div class="btn-toolbar mb-2 mb-md-0">
            <a href="{{route('admin.division.create')}}" class="btn btn-sm btn-outline-secondary">Add Division </a>
        </div>
</div>
                ...
                @if($divisions->count() > 0 ) 
                    ...
                    @foreach($divisions as $division) 
                        <tr>
                            ...
                            <td>
                            <a class="btn btn-info btn-sm" href="{{route('admin.division.edit',\Crypt::encrypt($division->id)) }}" title="Edit">Edit</a> |
                            <a class="btn btn-danger btn-sm" onclick="deleteRecord('{{$division->id}}')">Delete</a>
                                <form id="delete-{{$division->id}}" action="{{ route('admin.division.destroy',$division->id) }}" method="POST" style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </td>
