...
@else
@section('content')
<form action="{{route('admin.division.update',$division->id)}}" method="post"  autocomplete="off">
    @csrf
    @method('PUT')
    ...
    <div class="form-group row">
        <label for="form-control-label" class="col-sm-2 col-form-label">Name</label>
        <div class="col-sm-10">
            <input type="text" class="form-control" name="abbreviation" id="" value="{{$division->abbreviation}}">
        </div>
    </div>
