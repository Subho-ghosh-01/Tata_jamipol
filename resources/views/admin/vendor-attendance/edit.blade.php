...
@else
    @section('content')
        ...
        @if($vendor_attendance->account_by != '')
            @php
                $decision = strtolower($vendor_attendance->hr_decision ?? '');
            @endphp
            ...
            <div class="col-12">
                <div class="card shadow-sm rounded-4 mt-4">
                    ...
                    <div class="card-body">
                        <p class="mb-2"><strong>Account Name:</strong> {{ $account_name->name ?? 'NA' }}</p>

                        <p class="mb-2 d-flex align-items-center">
                            <strong class="me-2">Account Decision: </strong>
                            @if(isset($vendor_attendance->account_decision))
                                @if($vendor_attendance->account_decision === 'approve')
                                    &nbsp;
                                    <i class="fas fa-check-circle text-success me-1"></i>&nbsp;
                                    {{ ucfirst($vendor_attendance->account_decision) }}
                                @elseif($vendor_attendance->account_decision === 'reject')
                        ...
                        <p class="mb-2"><strong>Account Remarks:</strong> {{ $vendor_attendance->account_remarks ?? 'NA' }}</p>

                        <p class="mb-0">
                            <strong>Account Approve Datetime:</strong>
                            {{ isset($vendor_attendance->account_decision_datetime) ? date('d-m-Y H:i:s', strtotime($vendor_attendance->account_decision_datetime)) : 'NA' }}
                        </p>
                    </div>
                </div>
            </div>
        @endif
        @if(($vendor_attendance->status == 'Pending_with_hr' && Session::get('clm_role') == 'hr_dept') || ($vendor_attendance->status == 'Pending_with_account' && Session::get('clm_role') == 'Account_dept'))
            <div class="card-body material-card mt-4 shadow rounded-3" {!! $foemview ?? '' !!}>

                <form action="{{ route('admin.vendor_attendance.update', $vendor_attendance->id) }}" method="POST"
