...
@section('content')
    <div class="container-fluid py-4">
        ...
        <ul class="nav nav-tabs mb-3" id="misTabs" role="tablist">
            <li class="nav-item">
                <a href="{{ route('mis_report', ['id' => 'found']) }}"
                    class="nav-link {{ request()->routeIs('vendor_mis.mis_report') ? 'active' : '' }}">📑 Report</a>
            </li>
            <li class="nav-item">
                <a href="{{ route('mis_dashboard', ['id' => 'found']) }}"
                    class="nav-link {{ request()->routeIs('vendor_mis.mis_dashboard') ? 'active' : '' }}">📊 Dashboard</a>
            </li>
        </ul>
            ...
            <div class="card-body">
                <form id="filterForm">
                    ...
                    <div class="row g-3">
                        ...
                        {{-- Super Admin --}}
                        @if(Session::get('accoss_location') == 1)
                            <div class="col-md-4">
                            ...
                            {{-- Division-wise User --}}
                        @elseif(Session::get('division_wise') == 1)
                            <div class="col-md-4">
                                <label class="form-label">Plant</label>
                                <select class="form-control" name="plant" id="plant">
                                    <option value="">Select Plant</option>
                            ...
                            {{-- Plant-wise User --}}
                        @elseif(Session::get('plant_wise') == 1)
                            <div class="col-md-4">
                                <label class="form-label">Department</label>
                                <select class="form-control" name="department" id="department">
                                    <option value="">Select Department</option>
                            ...
                            {{-- Vendor --}}
                        @elseif(Session::get('vendor_safety_mis') == 1)
                            {{-- Vendor has no plant/division filter --}}
                        @endif

                        {{-- Common filters for all --}}
                        <div class="col-md-4">
                        ...
                        {{-- Vendor Filter - show only if not vendor themselves --}}
...
@section('scripts')
    ...
    <script>
        ...
        function loadVendors() {
            $.get("{{ route('admin.vendorGet_vendor_mis', 1) }}", function (vendors) {
        ...
        function loadSummary(filters = {}) {
            $.post("{{ route('admin.vendor_mis.filterJson_dashboard') }}",
                { _token: "{{ csrf_token() }}", ...filters },
                function (response) {
        ...
        function loadVendorComparison(filters = {}) {
            $.post("{{ route('admin.vendor_mis.filterJson_dashboard') }}",
                { _token: "{{ csrf_token() }}", vendor_comparison: true, ...filters },
                function (response) {
        ...
        function loadVendorComparisonTable() {
            ...
            Promise.all([
                $.post("{{ route('admin.vendor_mis.filterJson_dashboard') }}", { _token: "{{ csrf_token() }}", vendor_id: vendor1Id, ...baseFilters }),
                $.post("{{ route('admin.vendor_mis.filterJson_dashboard') }}", { _token: "{{ csrf_token() }}", vendor_id: vendor2Id, ...baseFilters })
