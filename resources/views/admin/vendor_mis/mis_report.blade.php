        ...
        <ul class="nav nav-tabs mb-3" id="vmsTabs" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <a href="{{ route('mis_report', ['id' => 'found']) }}"
                    class="nav-link {{ request()->routeIs('vendor_mis.mis_report') ? 'active' : '' }}" role="tab">
                    📑 Report
                </a>
            </li>
            ...
            <li class="nav-item" role="presentation">
                <a href="{{ route('mis_dashboard', ['id' => 'found']) }}"
                    class="nav-link {{ request()->routeIs('vendor_mis.mis_dashboard') ? 'active' : '' }}" role="tab">
                    📊 Dashboard
                </a>
            </li>
    ...
    <script>
        ...
        function loadTable(filters = {}) {
            ...
            table = $('#Table').DataTable({
                ...
                serverSide: false,
                ajax: {
                    url: "{{ route('admin.vendor_mis.filterJson') }}",
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
                        ...filters
        ...
        $(document).ready(function () {
            ...
            $('#exportExcel').click(function () {
                ...
                $.post("{{ route('admin.vendor_mis.filterJson') }}", { _token: "{{ csrf_token() }}", ...filters }, function (response) {
