...
<form action="" method="post" enctype="multipart/form-data" id="form_return" autocomplete="off">
    ...
    <a class="btn btn-primary px-4 py-2 rounded-pill col-12 text-white shadow-sm d-inline-flex align-items-center"
        ...
        <i class="bi bi-pencil-square me-2"></i> {{ $vms->full_sl }}
        ...
        <div class="container mt-4 pb-4">
            ...
            <div class="mb-4">
                <h3 class="fw-bold text-dark">
                    ...
                    {{ $type }} Vehicle Gate Pass Management System (Edit Driver Details)
                </h3>
            ...
            <div class="row g-3 mt-12">
                ...
                <div class="col-md-12">
                    ...
                    <input type="date" class="form-control hd" value="{{ $vms->insurance_valid_to }}"
                        name="insurance_to">
            ...
            <div class="form-group">
                ...
                <select class="form-control hd" name="vehicle_category" id="vehicle_category" required>
                    ...
                    @if($type == 'Employee')
                        <option {{$not_required ?? ''}} value="Diesel"
                            @if($vms->vehicle_type == 'Diesel'){{'selected'}}@endif>🛢️
                            Diesel</option>
                        <option {{$not_required ?? ''}} value="CNG" @if($vms->vehicle_type == 'CNG'){{'selected'}}@endif>🧯
                            CNG
                        </option>
                    @endif
                    <option value="EV" @if($vms->vehicle_type == 'EV'){{'selected'}}@endif>🔌 EV</option>
                    @if($type == 'Employee')
                        <option {{$not_required ?? ''}} value="Hybrid"
                            @if($vms->vehicle_type == 'Hybrid'){{'selected'}}@endif>♻️
                            Hybrid</option>
                    @endif
            ...
            <div class="row g-3 mt-2">
                ...
                <div class="col-md-12">
                    ...
                    <input type="date" class="form-control hd" value="{{ $vms->license_valid_to }}"
    ...
    <div class="card shadow-lg border-0 mb-4 rounded-3">
        ...
        <div class="card-body">
            <form id="driver_form" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="vms_id" value="{{$vms->id}}">
                <div class="form-group">
                    <label>Vehicle Will be Driven By</label>
                    <select class="form-control" id="driver_type" name="driver_type" required>
                        <option value="">--Select--</option>
                        <option value="self" @if($vms->driven_by == 'self') {{'selected'}} @endif>🙋‍♂️ Self</option>
                        <option value="driver" @if($vms->driven_by == 'driver') {{'selected'}} @endif>👤 Through Driver
                        </option>
                    </select>
                </div>
                ...
                <div id="driver_box" class="card shadow-sm border-0 mt-3" style="display: none;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="driver_name" value="{{ $vms->driver_name }}">
                    </div>

                </div>
                <div class="form-group">
                    <label>Driving License No.</label>
                    <input type="text" class="form-control" name="license_no" value="{{ $vms->driving_license_no }}">
                </div>
                    ...
                    <div class="form-row">
                        <div class="col">
                            <input type="date" class="form-control" name="license_valid_from"
                                value="{{ $vms->license_valid_from }}" required>
                        </div>
                        <div class="col">
                            <input type="date" class="form-control" name="license_valid_to"
                                value="{{ $vms->license_valid_to }}" required>
                        </div>
                    </div>
                </div>
                ...
                <a href="{{ asset($vms->driving_license_doc) }}" target="_blank" class="btn btn-outline-primary btn-sm">View
                </a>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function validateForm() {
            ...
            if (fileInput.files.length === 0 && !"{{ $vms->driving_license_doc ?? false }}") {
                Swal.fire({
                    icon: 'warning',
        ...
        previewBtn.addEventListener("click", function () {
            ...
            else if ("{{ $vms->driving_license_doc ?? false }}") {
                fileName = ``;
        ...
        `;
            ...
            fetch("{{ route('vms_ifream.update_driver_details') }}", {
                method: 'POST',
                headers: {
