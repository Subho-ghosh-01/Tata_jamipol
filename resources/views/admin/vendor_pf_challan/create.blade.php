...
@section('breadcrumbs')
    ...
    <li class="breadcrumb-item"><a href="{{route('admin.vendor_esic_details.index')}}">List Of Vendor PF Documents</a>
    </li>
...
@else
    ...
    @section('content')
        ...
        @endif
        @if(session('message'))
            <div class="alert {{ session('message') == 'PF uploaded successfully.' ? 'alert-success' : 'alert-danger' }}">
                {{ session('message') }}
            </div>
        @endif
        ...
        <form action="{{ route('admin.vendor_pf_details.store') }}" method="post" enctype="multipart/form-data" id="form"
            autocomplete="off">
            @csrf
