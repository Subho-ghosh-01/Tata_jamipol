...
<body class="py-5">
                            ...
                            <select class="form-select" name="silo_id" id="" required>
                                ...
                                @if($active_list->count() > 0)
                                    @foreach($active_list as $list)
                                        <option value="{{$list->id}}">{{$list->full_sl}}</option>
                                    @endforeach
                                @endif
                            </select>
                            <div class="invalid-feedback"></div>
    ...
    <script src="{{ asset('js/tevo.js') }}"></script>
    <script src="{{ asset('js/speechtotext.js') }}"></script>
    <!-- translate silo -->
    ...
    <script>
        $(document).ready(function () {
            $("#dynamicForm").on("submit", function (e) {
                ...
                $.ajax({
                    url: "{{ route('silo_daily_inspection.store') }}", // Your Laravel route
                    method: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
