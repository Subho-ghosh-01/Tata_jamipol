...
// Get latest half-yearly period from setting table
$half_yearly_month = sqlsrv_query($sqlcon, "SELECT TOP 1 value FROM settings WHERE type IN ('half_yearly1','half_yearly2') ORDER BY value DESC");
$month_data = sqlsrv_fetch_array($half_yearly_month);
$month = date('F Y', strtotime($month_data['value']));

// Set datetime range for the selected period month
$dateFrom = date('Y-m-01 00:00:00', strtotime($month_data['value']));
$dateTo = date('Y-m-t 23:59:59', strtotime($month_data['value']));



file_put_contents("debug_mail_log.txt", "=== MAIL LOG: " . date('Y-m-d H:i:s') . " ===\n");

// Get all vendors (sample with id 38)
...
while ($vendor = sqlsrv_fetch_array($vendor_list, SQLSRV_FETCH_ASSOC)) {
    ...
    // Check Half-Yearly Return entry
    $half_stmt = sqlsrv_query(
        $sqlcon,
        "SELECT id FROM vendor_hyr WHERE created_date BETWEEN ? AND ? AND vendor_id = ?",
        [$dateFrom, $dateTo, $vendor_id]
    );
    $half = sqlsrv_fetch_array($half_stmt);
    $has_half = !empty($half['id']);

    // SEND MAIL if not submitted
    if (!$has_half) {
        $status_rows = statusRow("Half Yearly Return", $has_half);
...
<head>
    <meta charset="UTF-8">
    <title>Vendor Half-Yearly Return Reminder</title>
</head>
...
<div style="max-width: 600px; margin: auto; background: #ffffff; border-radius: 10px; overflow: hidden; box-shadow: 0 0 12px rgba(0,0,0,0.08);">
    <div style="background: #800000; color: white; text-align: center; padding: 25px 20px;">
        <h2 style="margin: 0; font-size: 20px;">Vendor Half-Yearly Return Submission Reminder</h2>
    </div>
    <div style="padding: 25px; color: #333;">
        <p>Dear <strong>$vendor_name</strong>,</p>
        <p>This is a reminder to submit the <strong>Half-Yearly Return</strong> for the period of <strong>$month</strong>. Below is the current submission status:</p>
        ...
        <p style="margin-top: 20px;">Please upload the missing document via the SURAKSHA portal at your earliest convenience.</p>
        <p>If you have already submitted the document, kindly ignore this message.</p>
...
HTML;
        ...
        $subject = "Reminder: Half-Yearly Return Not Submitted - $month";
        $all_cc = array_filter([$executing_mail_cc]);

        sendSMTP($vendor_email, $from, 'HR Dept', $from, 'HR Dept', $subject, $body, $all_cc);

        file_put_contents("debug_mail_log.txt", "✅ Sent to: $vendor_email (ID: $vendor_id)\n", FILE_APPEND);
    } else {
