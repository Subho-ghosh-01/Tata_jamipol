...
while ($vendor = sqlsrv_fetch_array($vendor_list, SQLSRV_FETCH_ASSOC)) {
    ...
    // Check Wage Register entry
    $wage_register_stmt = sqlsrv_query(
        $sqlcon,
        "SELECT id FROM vendor_attendance_upload WHERE created_date BETWEEN ? AND ? AND vendor_id = ?",
        [$dateFrom, $dateTo, $vendor_id]
    );
    $wage_register = sqlsrv_fetch_array($wage_register_stmt);
    $has_wage_register = !empty($wage_register['id']);




    // SEND MAIL if ANY ONE is MISSING
    if (!$has_wage_register) {
        $status_rows = '';
        $status_rows .= statusRow("Wage Register", $has_wage_register);
        $status_rows .= statusRow("Bank Details", $has_wage_register);
