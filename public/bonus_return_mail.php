...
$bonus_month = sqlsrv_fetch_array(sqlsrv_query($sqlcon, "Select value from settings Where type = 'bonus_month'"));

$month = date('F Y', strtotime($bonus_month['value']));
...
while ($vendor = sqlsrv_fetch_array($vendor_list, SQLSRV_FETCH_ASSOC)) {
    ...
    // Check Bonus Return entry
    $bonus_stmt = sqlsrv_query(
        $sqlcon,
        "SELECT id FROM vendor_ecm WHERE created_date BETWEEN ? AND ? AND vendor_id = ?",
        [$dateFrom, $dateTo, $vendor_id]
    );
    $bonus = sqlsrv_fetch_array($bonus_stmt);
    $has_bonus = !empty($bonus['id']);
    ...
    // SEND MAIL if Bonus not submitted
    if (!$has_bonus) {
        $status_rows = '';
        $status_rows .= statusRow("Bonus Return Form", $has_bonus);
...
<head>
    <meta charset="UTF-8">
    <title>Vendor Bonus Return Reminder</title>
</head>
...
<div style="max-width: 600px; margin: auto; background: #ffffff; border-radius: 10px; overflow: hidden; box-shadow: 0 0 12px rgba(0,0,0,0.08);">
    <div style="background: #800000; color: white; text-align: center; padding: 25px 20px;">
        <h2 style="margin: 0; font-size: 20px;">Vendor Bonus Return Submission Reminder</h2>
        
    </div>
    <div style="padding: 25px; color: #333;">
        <p>Dear <strong>$vendor_name</strong>,</p>
        <p>This is a reminder to submit the <strong>Bonus Return Form</strong> for the month of <strong>$month</strong>. Below is the current submission status:</p>
...
HTML;
        ...
        $subject = "Reminder: Bonus Return Not Submitted - $month";
